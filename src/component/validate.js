...
class Menu extends React.Component {
  ...
  validateCycle = () => {
    ...
    addStat(data).then((res) => {
      getStateByWeek(
        moment(new Date()).format("W"),
        this.props.user.subuser[i].id,
        this.props.theme.allTheme
      ).then((resultstate) => {
        this.props.loadProgress(resultstate, this.props.progress.obj);

        let current_state = resultstate.filter(
          (state) => state.id === this.props.theme_id
        )[0].state;
        let current_obj = this.props.progress.obj.filter(
          (obj) => obj.id === this.props.theme_id
        )[0].obj;

        if (current_state === current_obj) {
    ...
    return (
      <View style={styles.container}>
        ...
        {this.state.showVideo && (
          <Video
            source={{
              uri: "https://res.cloudinary.com/dmpzubglr/video/upload/v1614688789/general/validation%20award/Validation_semaine-720p-210227_bmhovt.mp4",
            }}
            rate={1.0}
            volume={1.0}
            isMuted={false}
