...
export const getAllAwards = async (subuser_id) => {
  ...
  return axios
    .get(config.api_url + "/api/v1/all/award/" + subuser_id, {
...
export const getAwardById = async (id, subuser_id) => {
  const token = await AsyncStorage.getItem("4brntoken");
  return axios
    .get(config.api_url + "/api/v1/get/award/" + id + "/" + subuser_id, {
      headers: { "x-access-token": token },
    })
    .then((response) => {
      return response.data;
...
export const saveNewAward = async (datas) => {
  const token = await AsyncStorage.getItem("4brntoken");
  return axios
    .post(config.api_url + "/api/v1/add/award", datas, {
      headers: { "x-access-token": token },
    })
    .then((res) => {
      return res.data;
...
export const deleteImage = (name) => {
  return axios
    .delete(config.api_url + "/api/v1/delete/award/pict/" + name, {
      headers: { "x-access-token": token },
    })
    .then((res) => {
      return res.data;
